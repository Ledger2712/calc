...
def calculate_retail_book_price(
    ...
    format_coefficients = {
        "Def": 2.0,   # twice the paper area of A5
        "Pro": 1.0,   # base format
        "Max": 0.5    # half the paper area of A5
    }

    if format_code not in format_coefficients:
        raise ValueError("format_code must be 'Def', 'Pro' or 'Max'")
...
st.set_page_config(page_title="Калькулятор цены цацуацуа книги", layout="centered")

st.title("📚 Калькулятор розничной цены книги")
st.markdown("---")
...
with st.form("price_calculator_form"):
    ...
    with col3:
        format_code = st.selectbox(
            "Модель телефона",
            options=["Def", "Pro", "Max"],
            help="Выберите модель: Def (обычная), Pro (улучшенная), Max (максимальная)"
        )
